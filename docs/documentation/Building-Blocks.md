...
| Application Handlers    | AWS SSO groups event handler                | lib/lambda-functions/application-handlers/src/groupsCud.ts                      | Handler to process AWS SSO group events                                                                  | AWS SSO group events rule (AWS SSO event) through cross-account topic                   | Account assignment queue/ org entities state machine(org account) |
| Application Handlers    | AWS SSO users event handler                 | lib/lambda-functions/application-handlers/src/usersCud.ts                       | Handler to process AWS SSO user events                                                                   | AWS SSO user events rule (AWS SSO event) through cross-account topic                    | Account assignment queue/ org entities state machine(org account) |
...
| Current config Handlers | Custom resource handler                     | lib/lambda-functions/current-config-handlers/src/triggerParentSM.ts             | Cloud formation custom resource handler for importing current AWS SSO<br>configuration into the solution | Cloud formation                                                                         | Current configuration<br>state machine(AWS SSO account)           |
...
| Current config Handlers | Permission set import handler               | lib/lambda-functions/current-config-handlers/src/import-permission-sets.ts      | Handler to import permission sets into the solution                                                      | Import permission set state machine(AWS SSO account) through cross-account<br>topic     | n/a                                                               |
| Current config Handlers | Account assignment import handler           | lib/lambda-functions/current-config-handlers/src/import-account-assignment.ts   | Handler to import account assignments into the solution                                                  | Import account assignment state machine(AWS SSO account) through cross-account<br>topic | n/a                                                               |
...
| SSO groups event topic                 | SSO      | SSO groups events event bridge rule        | AWS SSO groups event handler(target)            |
| SSO users event topic                  | SSO      | SSO users events event bridge rule         | AWS SSO users event handler(target)             |
...
| ------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| Org entities list         | Org main    | AWS SSO groups event handler,<br>AWS SSO users event handler,<br>Permission set sync handler,<br>Account assignment topic processor | Resolve target accounts by querying org API's                        |
